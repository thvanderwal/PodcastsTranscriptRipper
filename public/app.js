// Frontend JavaScript for Podcast Transcript Exporter
// This version connects to Vercel serverless API endpoints

let currentTranscript = null;

// API URL - automatically detects if running locally or on Vercel
...
async function fetchTranscript() {
  ...
  // Validate URL format
  if (!url.includes('podcasts.apple.com') || !url.includes('?i=')) {
    showError('Please enter a valid Apple Podcasts episode URL (must include ?i= parameter)');
    return;
  }
...
function displayResults(data) {
  ...
  const metadataHTML = `
    ...
    <div class="metadata-item">
      ...
      <span class="metadata-value">${escapeHtml(metadata.duration)}</span>
    </div>
    <div class="metadata-item">
      <span class="metadata-label">Episode ID:</span>
      <span class="metadata-value">${escapeHtml(metadata.episodeId)}</span>
    </div>
